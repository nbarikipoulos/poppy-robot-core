...
class RawMotorRequest {
  ...
  */
  async set (registerName, data) {
    return this._reqHandler.setMotorRegister(
      this._motor.name,
      registerName,
      data
    )
  ...
  */
  async get (...registerNames) {
    return (await Promise.all(registerNames.map(
      async reg => this._reqHandler.getMotorRegister(
        this._motor.name,
        reg
      )
  ...
  */
   ...
   * @return {Array.<string>}
  */
  getLedValues () { // FIXME TO REMOVE
    return [].concat(..._LED_VALUES)
  }
}

const _LED_VALUES = [ // FIXME TO REMOVE
  'off',
  'red',
  'green',
  'blue',
