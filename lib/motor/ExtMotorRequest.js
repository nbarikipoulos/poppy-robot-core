...
class ExtMotorRequest extends RawMotorRequest {
  ...
  */
   ...
   * @param {boolean=} [safe=true] - Init or not the 'goal_position' register value to
   *  the 'present_position' one when switching the compliant stat to __false__
   * @return {Promise.<null>}
  */
  async setCompliant (value, safe = true) {
    if (!value && safe) {
      const reg = 'present_position'
      const pos = (await this.get(reg))[reg]
  ...
  */
  ...
  async _waitGoTo (targetValue) {
    ...
    const currentPos = await f('goal_position')
    ...
    const checkValue = _ => _wait(100, period)
      .then(async _ => f('present_position'))
      .then(result => {
        console.log('=>', result)
