 ...
 *
 * @param {?module:poppy-robot-core~DescriptorLocator} locator - The descriptor locator. Set to 'desc://live-discovering' if undefined or null.
 ...
 */
const createDescriptor = async (locator, connect) => {
  const descriptor = locator === LIVE_DISCOVERING || !locator
    ? await discoverDescriptor(connect)
    : readJSONDescriptor(locator)
  return descriptor
}
...
const discoverDescriptor = async (connect) => {
  ...
  try {
    ...
    throw new Error(`Unable to live discovering robot:\n${error}`)
  }
...
const readJSONDescriptor = (descriptorLocator) => {
  ...
  switch (schema) {
    ...
    default:
      ...
      throw new Error(
        `Invalid file descriptor locator '${descriptorLocator}'.\n` +
...
module.exports = {
  DEFAULT_DESCRIPTOR: LIVE_DISCOVERING,
  createDescriptor
}
