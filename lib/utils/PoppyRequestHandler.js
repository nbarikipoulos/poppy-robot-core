...
class PoppyRequestHandler {
  ...
  */
   ...
   * @return {Promise.<null>}
   */
  async setMotorRegister (motorName, registerName, data) {
    const [url, method] = _getRelativeURL(
      'set_register',
      {
    ...
    return this._getBase()[method](url, data)
  }
   ...
   *  // {'present_position': 15}
   */
  async getMotorRegister (motorName, registerName) {
    const [url, method] = _getRelativeURL(
      'get_register',
      {
        motor_name: motorName,
...
const _getBaseURL = (schema, ip, port) => `${schema}://${ip}:${port}`

const _getRelativeURL = (
  id,
  inputParameters = {}
) => {
  const descriptor = _REST_PATHS.find(elt => id === elt.id)

  // Check parameters, if any
  const pathParameters = descriptor.path.match(/:\w+/g) || []

  let path = descriptor.path
  pathParameters.forEach(param => {
    const value = inputParameters[param.slice(1)]
    if (value) {
...
PoppyRequestHandler.prototype.led = async function (motorName, color) {
  ...
  return this._getBase('snap')[method](url)
}
