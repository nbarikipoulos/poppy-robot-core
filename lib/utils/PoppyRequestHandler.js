...
class PoppyRequestHandler {
   ...
   * @return {Object}
   */
  getClientConfig (type = 'http') {
    let config

    switch (type) {
      case 'snap':
        config = this._axiosConf.snap
        break
      case 'http':
      default:
        config = this._axiosConf.http
    }
   ...
   * @param {'http'|'snap'=} [client='http'] - target server selector (http or snap)
   * @param {string=} [method='get'] - request method to be used when making the request
   * @param {Object=} - extra axios client settings
   ...
   * @return {Promise.<null>}
   */
  async setRegister (motorName, registerName, data) {
    return this.perform(
      `/motor/${motorName}/register/${registerName}/value.json`,
      { method: 'post', config: { data } }
    )
  }
   ...
   *  // {'present_position': 15}
   */
  async getRegister (motorName, registerName) {
    const result = await this.perform(`/motor/${motorName}/register/${registerName}`)
    return result.data
  }
