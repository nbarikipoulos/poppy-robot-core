...
class Poppy {
  ...
  */
   ...
   * @param {Array.<integer>|integer} position - target position: Either an array containing
   ...
   * )
  */
  async goto (motorNames, positions, duration, wait = false) {
    const poppy = this
    const motors = this.toMotorNames(motorNames)

    const gotoRequest = _ => poppy.requestHandler.post(
      '/motors/goto.json',
      {
        motors,
        positions: Array.isArray(positions) ? positions : Array(motors.length).fill(positions),
