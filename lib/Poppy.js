...
class Poppy {
     ...
     */
    async queryMotors(motorIds, registers) {

        let targetMotorIds = 'all' === motorIds?
            this._all:
            motorIds
        ;

        return (await Promise.all(
            targetMotorIds.map( async motor_name => {
              let motor = this.getMotor(motor_name);
              let data = await motor.get(...registers);
              return { [motor_name]: data};
            })))
     ...
     */
    async exec(...scripts) {
       let engine = new ScriptEngine(this);
